...
// FIX: optimize function using oneDNN
...
void linear_q(float *xout, QuantizedTensor *qx, int8_t *p, float *sf,
	      LinearConfigQ lc)
{
...
void conv_q(float *xout, QuantizedTensor *qx, int8_t *p, float *sf,
...
void head(Runstate *s, float *images, int8_t *p, float *sf, ConvConfigQ *cc,
...
void tail(Runstate *s, int8_t *p, float *sf, ConvConfigQ *cc,
...
void bottleneck(Runstate *s, int8_t *p, float *sf, ConvConfigQ *cc,
		ActivationConfigQ *ac, int *h, int *w, int *i, int *ia,
		bool downsample)
{
	float *x = s->x;
