...
void read_checkpoint(char* path, ModelConfig* config, ConvConfig** cl, LinearConfig** ll, float** parameters, int* fd, float** data, size_t* file_size) {
    // file should be ordered ModelConfig -> ConvConfig -> LinearConfig -> parameters
    ...
    *ll = (LinearConfig*) (*data + (config->nconv * sizeof(ConvConfig) + sizeof(ModelConfig)) / sizeof(float));
    // memory map weights and biases
    int header_size = sizeof(ModelConfig) + config->nconv * sizeof(ConvConfig) + config->nlinear * sizeof(LinearConfig);
...
void build_model(Model* m, char* checkpoint_path) {
    ...
    read_checkpoint(checkpoint_path, &m->model_config,&m->conv_config, &m->linear_config, &m->parameters, &m->fd, &m->data, &m->file_size);
...
void matmul_conv_with_relu(float* xout, float* x, float* p, int nchannels, int in, int out) {
...
void im2col_cpu(float* col, float* im, int nchannels, int height, int width, int ksize, int stride, int pad) {
    ...
    for (c = 0; c < col_size; c++) {
        ...
        for (h = 0; h < out_height; h++) {
            for (w = 0; w < out_width; w++) {
                ...
                col[col_index] = im2col_get_pixel(im, height, width, input_row, input_col, channel, pad);
            }
        }
    }
}
...
void forward(Model* m, uint8_t* image) {
    ...
    normalize(x2, image);
    im2col_cpu(x, x2, cl[0].ic, 28, 28, cl[0].ksize, cl[0].stride, cl[0].pad);
    matmul_conv_with_relu(x2, x, p + cl[0].offset, cl[0].oc, cl[0].ic*cl[0].ksize*cl[0].ksize, 28*28);
    maxpool(x2, 28, 28, cl[0].oc, 2);
    im2col_cpu(x, x2, cl[1].ic, 14, 14, cl[1].ksize, cl[1].stride, cl[1].pad);
    matmul_conv_with_relu(x2, x, p + cl[1].offset, cl[1].oc, cl[1].ic*cl[1].ksize*cl[1].ksize, 14*14);
