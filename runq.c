...
static void forward(Model * m, float *image)
{
	...
	im2col_q(x, image, &h, &w, cc[0]);
	quantize2d(&xq, x, cc[0], h * w);
	conv_q(x2, &xq, p, sf, cc[0], h * w, false);
	...
	// block 1.1 and 1.2
	for (int i = 1; i < 4; i += 2) {
		im2col_q(x3, x, &h, &w, cc[i]);
		quantize2d(&xq, x3, cc[i], h * w);
		conv_q(x, &xq, p, sf, cc[i], h * w, false);
		batchnorm(x3, x, sf + bc[i].offset, bc[i].ic, h * w, true);
		im2col_q(x, x3, &h, &w, cc[i + 1]);
		quantize2d(&xq, x, cc[i + 1], h * w);
		conv_q(x3, &xq, p, sf, cc[i + 1], h * w, false);
	...
	for (int i = 5; i < m->model_config.nconv - 4; i += 5) {
		...
		im2col_q(x3, x2, &h_prev, &w_prev, cc[i + 2]);
		quantize2d(&xq, x3, cc[i + 2], h * w);
		conv_q(x2, &xq, p, sf, cc[i + 2], h * w, false);
		...
		// block i.2
		im2col_q(x3, x, &h, &w, cc[i + 3]);
		quantize2d(&xq, x3, cc[i + 3], h * w);
		conv_q(x, &xq, p, sf, cc[i + 3], h * w, false);
		batchnorm(x3, x, sf + bc[i + 3].offset, bc[i + 3].ic, h * w,
			  true);
		im2col_q(x, x3, &h, &w, cc[i + 4]);
		quantize2d(&xq, x, cc[i + 4], h * w);
		conv_q(x3, &xq, p, sf, cc[i + 4], h * w, false);
